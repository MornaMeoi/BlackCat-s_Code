...
void branches_traversal(bst *root, int *first_flag);
...
int main()
{
    ...
    int first_flag = 1;
    branches_traversal(root, &first_flag);
    destroy(root);
    return 0;
}
...
{
    if(root->left != NULL)
        branches_traversal(root->left, first_flag);
    ...
    if(root->right != NULL)
        branches_traversal(root->right, first_flag);
}
