...
void left_traversal(bst *root, int first_flag);
...
{
    if(root->left != NULL)
        left_traversal(root->left, first_flag);
    if(!(root->left == NULL && first_flag))
        putchar('\n');
    first_flag = 0;
    printf("%d", root->value);
    if(root->right != NULL)
        left_traversal(root->right, first_flag);
}
